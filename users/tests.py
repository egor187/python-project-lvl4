...
class UserViewTest(TestCase):
    ...
    def test_create_user_within_form(self):
        ...
        self.client.post(url, {'username': 'David', 'password1': 'level5level5', 'password2': 'level5level5'})

        response_after_update = self.client.get(reverse('users:user_list'))
        self.assertEqual(response_after_update.status_code, 200)
        self.assertContains(response_after_update, CustomUser.objects.get(username='David'))
