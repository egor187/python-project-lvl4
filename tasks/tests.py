...
class StatusViewTest(TestCase):
    ...
    def test_delete_status(self):
        ...
        self.assertQuerysetEqual(response_after_delete.context['object_list'], [])
...
class TaskViewTest(TestCase):
    def setUp(self):
        ...
        task = Task.objects.create(name='JUMP', description='JUMP AROUND', task_status=status, creator=user, assigned_to=user)
        # task.label.create(name='new_label')
...
class LabelViewTest(TestCase):
    ...
    def test_create_label(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertQuerysetEqual(response.context['object_list'], ['<Label: new_label>'])
        label = Label.objects.create(name='another_label')
        label.save()
        response_after = self.client.get(url)
        self.assertEqual(response_after.status_code, 200)
        self.assertQuerysetEqual(response_after.context['object_list'], ['<Label: another_label>', '<Label: new_label>'])
    ...
    def test_update_label(self):
        ...
        self.assertQuerysetEqual(response_list.context['object_list'], ['<Label: new_label>'])
        ...
        self.assertContains(response_list_after, Label.objects.get(name='old_label'))
        self.assertQuerysetEqual(response_list_after.context['object_list'], ['<Label: old_label>'])
    ...
    def test_delete_label(self):
        ...
        response_list_after = self.client.get(url_list)
        self.assertEqual(response_list_after.status_code, 200)
        self.assertQuerysetEqual(response_list_after.context['object_list'], [])
    ...
    def test_create_label_within_form(self):
        ...
        self.assertEqual(response_after_create.status_code, 200)
        self.assertContains(response_after_create, Label.objects.get(name='DANCE_LABEL'))